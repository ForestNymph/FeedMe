...
public class AddedProductDataSource {
    ...
    private String[] statusColumns = {AddedProductDB_SQL.COLUMN_ID_PROD_STATUS,
            AddedProductDB_SQL.COLUMN_DATE_STATUS, AddedProductDB_SQL.COLUMN_STATUS,
            AddedProductDB_SQL.COLUMN_NAME_PROD_STATUS};
    ...
    public int modifyAmountOfAddedProduct(Product product, double amount1) {
        ...
        content.put(AddedProductDB_SQL.COLUMN_AMOUNT, amount1);
        int tmp = mDatabase.update(AddedProductDB_SQL.TABLE_PRODUCTS_ADDED, content, "_id_prod" + "=?",
                new String[]{String.valueOf(product.id)});
        return tmp;
    }

    public Product createAddedProduct(String date, String type, String name, int def1, int def2, int def3, double kcal, double protein,
